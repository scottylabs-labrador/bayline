...
// streaming past, the desk and the train-control screens glowing in the lower third. The eye a touch right of and
// below the driver's, 50 degrees, pitched down 9 degrees, a slow 25 cm push; 120 fps (the edit averages 4 frames) (TRAINS' look: _metrolook.mjs cabLook, exposure and the screens'
...
const S = 27400, FOV = 50, PITCH = -9.0, X0 = 0.02, X1 = 0.27;
export default {
  hash: '#auto&t=21:35&q=ultra&w=clear', warm: 45, frames: 960, fps: 120,
  ...
  prime: `async () => { const B = window.__bayline, d = window.__dep; B.Env.setClock(d.t); B.MetroSim.setFocus(d.key);
    ...
    ${cabLook ? `await (${cabLook})(d.key);` : ''} return d.key + ' ' + d.line + ' to ' + d.dest + ' ' + d.v + ' m/s'; }`,
  before: `(t) => { const B = window.__bayline; B.Env.camera.fov = ${FOV}; B.MetroSim.setFocus(window.__dep.key); }`,
  // the eye: 8 cm below TRAINS' spot, 10 cm back at the start, then an eased 25 cm push toward the windscreen, so the
  // tunnel grows and the screens slide down out of the band at the very end
  cam: `(t, cam) => { const C = __cine, M = window.__m, k = C.ease(t / 8), c = M.cab(window.__dep.key, ${X0} + (${X1} - ${X0}) * k, -0.08, 0.1); if (!c) return;
