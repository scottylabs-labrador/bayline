// P11, golden hour: the airport connector's cable train glides toward us on its white guideway beside the Nimitz
// freeway, the rush-hour traffic streaming below, the East Bay hills warm in the last sun. A drone 30 m up on the
// guideway's east side, 40 mm, looking north-northwest up the line; the little three-car train comes down it toward
// the right third, the sun raking in from the left. A slow eased drift toward it.
import { cine } from './_lib.mjs';
import { metro } from './_metro.mjs';
const S = 1300, SIDE = -70, H = 30, FOV = 28.4;
export default {
  ...
  setup: `async () => { ${cine}; ${metro}; const M = window.__m, B = window.__bayline, C = __cine;
    ...
    const hd = Math.atan2(F.tx, -F.tz) + 150 * Math.PI / 180, q = { x: c0.x + Math.sin(hd) * 300, z: c0.z - Math.cos(hd) * 300 }; q.y = c0.y - Math.tan(9 * Math.PI / 180) * 300;
    window.__P = { c0, c1: { x: c0.x + Math.sin(hd) * 6, y: c0.y - 1, z: c0.z - Math.cos(hd) * 6 }, q };
    C.put(c0, q);
    window.__dep = M.pass({ track: 'H-main.2', s: ${S} - 95 }, { kind: 'apm' }, 18 * 3600 + 31 * 60, 2.5, { focus: true });
    return window.__dep; }`,
