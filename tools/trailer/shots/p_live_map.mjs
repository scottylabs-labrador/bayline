...
export default {
  ...
  prime: `async () => { const B = window.__bayline, U = B.MetroUI; B.Env.setClock(21 * 3600 + 5 * 60); U.openMap({ view: 'geo' });
    ...
    const fit = Math.min(c.width / ((x1 - x0) * 1.12), c.height / ((z1 - z0) * 1.12)), wye = __cine.ll(37.8015, -122.2800, 0);
    window.__MV = { a: { cx: wye.x, cz: wye.z, scale: fit * 2.4 }, b: { cx: (x0 + x1) / 2, cz: (z0 + z1) / 2, scale: fit } };
    Object.assign(m.geo, window.__MV.a); B.Env.time.scale = ${SCALE}; return JSON.stringify(window.__MV); }`,
