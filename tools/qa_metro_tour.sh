...
# Usage: [PORT=8136] [DWELL=11000] [AUTO_MIN=10] [XH=extra&hash] sh tools/qa_metro_tour.sh [page] [outdir]     one headless Chrome at a time
set -u
ROOT=$(cd "$(dirname "$0")/.." && pwd); cd "$ROOT"
PAGE=${1:-sim.html}; OUT=${2:-/tmp/bayline-metro-tour}; mkdir -p "$OUT"
BASE="http://localhost:${PORT:-8136}/$PAGE"; DWELL=${DWELL:-11000}; AUTO_MIN=${AUTO_MIN:-10}; X=${XH:+&$XH}   # XH=metrodir=metro-next/ adds to every URL
fails=0
# 1. the station tour (about 55 stops x (DWELL + 0.7 s))
python3 tools/wd.py 2400 node tools/shot.mjs "$BASE#auto&metro=1&t=08:00$X" "$OUT/tour.png" --gpu --w 1280 --h 800 --wait 300 \
...
if grep -qE '^\[pageerror\]|^\[console\.error\]|^\[eval error\]' "$OUT/tour.log"; then echo "FAIL tour: page or console errors:"; grep -E '^\[pageerror\]|^\[console\.error\]' "$OUT/tour.log" | sort | uniq -c | head -20; fails=$((fails+1)); else echo "PASS tour: no page or console errors"; fi
...
CYCLE="new Promise(r=>{const f=()=>window.__bayline&&__bayline.MetroSim&&__bayline.MetroSim.ready?r():setTimeout(f,300);f();}).then(()=>{const B=__bayline,K=(c)=>{window.dispatchEvent(new KeyboardEvent('keydown',{code:c,key:c,bubbles:true}));window.dispatchEvent(new KeyboardEvent('keyup',{code:c,key:c,bubbles:true}));};
...
python3 tools/wd.py $((AUTO_MIN * 60 + 300)) node tools/shot.mjs "$BASE#auto&metro=1&t=17:30$X" "$OUT/auto.png" --gpu --w 1280 --h 800 --wait 300 --eval "$CYCLE" > "$OUT/auto.log" 2>&1
grep '^\[eval\]' "$OUT/auto.log" | tail -1 | cut -c1-300
grep -q '"metroOn":true' "$OUT/auto.log" || { echo "FAIL auto: the metro went off"; fails=$((fails+1)); }
if grep -qE '^\[pageerror\]|^\[console\.error\]|^\[eval error\]' "$OUT/auto.log"; then echo "FAIL auto: page or console errors:"; grep -E '^\[pageerror\]|^\[console\.error\]' "$OUT/auto.log" | sort | uniq -c | head -20; fails=$((fails+1)); else echo "PASS auto: $AUTO_MIN min, no page or console errors"; fi
