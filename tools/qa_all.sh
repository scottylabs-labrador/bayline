...
# Usage: [PORT=8123] [METRO=1|0] [XH=k=v&...] sh tools/qa_all.sh [page] [outdir]      page defaults to lead.html (tools/devbuild.sh output)
...
MQ="$MQ${XH:+&$XH}"                               # XH=metrodir=metro-next/ (any extra hash) adds to every view too
[ -n "$MQ" ] && echo "== with #${MQ#&}"
...
shot() { name=$1; shift; node tools/shot.mjs "$@" "$OUT/$name.png" --gpu --w 1440 --h 900 2>&1 | tee "$OUT/$name.log" | grep -E "\[eval\]|error|Error" | sed "s/^/[$name] /" | cut -c1-400; }   # (full output: $OUT/<name>.log)
echo "== views"
...
echo "== drive (keyboard only: the Short Hop, Mountain View - Sunnyvale - Lawrence, to its report card)"
# (waits for the run to end, up to 5 min; the summary's short fields come first: maxMph, done, stops, missed, score, grade)
shot drive_qa "$BASE#auto$MQ&t=08:00" --wait 20000 --eval "$(cat tools/qa_drive.js)" --eval2 "new Promise(r=>{const t0=Date.now(),f=()=>window.__qa.done||Date.now()-t0>300000?r():setTimeout(f,500);f();}).then(()=>window.__qaDriveSummary())"
echo "== ride flow"
