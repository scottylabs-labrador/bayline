...
const FMissions = (() => {
  ...
  async function start(id) {
    ...
    if (m.air) {
      ...
      ac.place({ x: w.x, y: Math.max(m.air.alt, gy + 120), z: w.z, hdg, pitch: 2 * D, fpa: 0, speed: m.air.kt * KT, gear: 0, flaps: 0, thr: m.type === 'f16' ? 0.9 : 0.75 });
      Flight.fcs.airStart(0); Flight.fcs.ap.athr = Flight.fcs.assist !== 'direct'; Flight.fcs.ap.spd = m.air.kt; Flight.fcs.ap.thrI = 0.7; Flight.cam.reset();
    } else {
