...
const ACParts = (() => {
  ...
  function lights(type, ctx, env) {
    ...
    env.anim.push((ac, dt, st) => {
      ...
      // (from the cockpit the landing and taxi lights are not seen themselves, only what they light: their glows shrink
      // away, but stay 'visible', which is what FVfx's light beams follow)
      const glow = st.inside ? 0.001 : 1;
      lit(L.landL, landOn, (0.6 + night) * glow); lit(L.landR, landOn, (0.6 + night) * glow);
      lit(L.taxi, ac.gearPos > 0.95 && (ac.out.gs > 1 || landOn) && eng1, (0.5 + night) * glow);
