...
const ACFrame = (() => {
  ...
  const NOSES = {
    airbus: { tip: 0.36, top: [[0, 0], [0.05, 0.13], [0.12, 0.22], [0.2, 0.29], [0.28, 0.35], [0.36, 0.44], [0.44, 0.55], [0.52, 0.66], [0.62, 0.77], [0.75, 0.88], [0.88, 0.96], [1, 1]],
    ...
    b737: { tip: 0.4, top: [[0, 0], [0.05, 0.1], [0.15, 0.16], [0.3, 0.21], [0.46, 0.27], [0.56, 0.44], [0.66, 0.61], [0.78, 0.8], [0.9, 0.94], [1, 1]],
    ...
    b787: { tip: 0.39, top: [[0, 0], [0.04, 0.14], [0.1, 0.24], [0.18, 0.31], [0.26, 0.37], [0.34, 0.46], [0.42, 0.57], [0.52, 0.7], [0.64, 0.83], [0.8, 0.94], [1, 1]],
    ...
    a380: { tip: 0.33, top: [[0, 0], [0.05, 0.12], [0.12, 0.18], [0.2, 0.22], [0.27, 0.26], [0.34, 0.33], [0.42, 0.43], [0.52, 0.56], [0.64, 0.7], [0.78, 0.86], [0.9, 0.96], [1, 1]],
    ...
    concorde: { tip: 0.47, top: [[0, 0], [0.15, 0.025], [0.3, 0.045], [0.45, 0.065], [0.6, 0.09], [0.66, 0.2], [0.74, 0.45], [0.82, 0.68], [0.9, 0.86], [1, 1]],
    ...
    bizprop: { tip: 0.4, top: [[0, 0], [0.12, 0.05], [0.3, 0.08], [0.5, 0.1], [0.6, 0.13], [0.7, 0.4], [0.8, 0.66], [0.9, 0.87], [1, 1]],
  ...
  function cockpitPanes(H, N) {
    ...
    const panes = [];
    const ray = (az, el) => { const a = az * D, e = el * D; return H.hit(eye, new V3(Math.cos(e) * Math.cos(a), Math.sin(e), -Math.cos(e) * Math.sin(a)).normalize()); };   // + azimuth: outboard (left)
    // the azimuth, at an elevation, of the ray that meets the skin on the centre line (the centre post)
    const toPost = (el) => { let lo = -75, hi = 20; for (let i = 0; i < 24; i++) { const mid = (lo + hi) / 2, h = ray(mid, el); if (!h || h.p.z > 0) lo = mid; else hi = mid; } return (lo + hi) / 2; };
    N.panes.forEach((P, k) => {
      // (the front pane's inboard corners are on the centre post, whatever the spec's azimuths say)
      const onPost = [];
      const pts = P.map(([az, el], i) => {
        let h = null;
        if (k === 0 && (i === 0 || i === 3)) { h = ray(toPost(el), el); if (h && Math.abs(h.p.z) > 0.08) h = null; onPost[i] = !!h; }     // (the ray reached the post)
        if (!h) h = ray(az, el);
        return h ? [h.s, h.th] : null;
      });
      if (pts.some(p => !p)) return;
      // (the captain's side is th in (pi, 2 pi): mirror to the right side, th -> 2 pi - th)
      let poly = pts.map(([s, th]) => [s, 2 * Math.PI - th]);
      if (k === 0) {       // the front pane's inboard edge runs along the centre post (where its rays reached it)
        for (const i of [0, 3]) if (onPost[i]) poly[i] = [poly[i][0], N.post / Math.max(0.3, H.sec(poly[i][0]).a) + 0.004];
        // (over a tall nose the ray down the post runs on almost to the tip: then the lower edge is kept near level,
        // the inboard corner on the post at the outboard corner's height)
        const p0 = new V3(), p1 = new V3(); H.pt(poly[0][0], poly[0][1], p0); H.pt(poly[1][0], poly[1][1], p1);
        if (p0.y < p1.y - 0.2) {
          const yT = p1.y - 0.08; let lo = 0.05, hi = poly[3][0];
          for (let i = 0; i < 30; i++) { const mid = (lo + hi) / 2, th = N.post / Math.max(0.3, H.sec(mid).a) + 0.004; H.pt(mid, th, p0); if (p0.y > yT) hi = mid; else lo = mid; }
          poly[0] = [(lo + hi) / 2, N.post / Math.max(0.3, H.sec((lo + hi) / 2).a) + 0.004];
        }
      }
